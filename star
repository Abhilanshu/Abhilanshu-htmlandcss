...
<body>
    ...
    <div class="performance-warning">
        <i class="fas fa-exclamation-triangle"></i> Performance mode enabled for mobile
    </div>
    ...
    <div class="orrery-info-panel">
        ...
        <p>A simplified model of our solar system. Drag to rotate, scroll to zoom. Observe the planets orbiting the sun.</p>
    </div>
    ...
    <div class="story-content">
        ...
        <section class="solar-system-section">
            <div class="solar-system-content">
                ...
                <p class="solar-system-text">As we journey beyond the nebula, we encounter our solar system with all eight planets orbiting the sun. Drag to rotate, scroll to zoom, and observe the intricate dance of planets.</p>
            </div>
        </section>
    ...
    <script type="module">
        ...
        const isLowEndDevice = () => {
            ...
            // Check for mobile devices with small screens (typically lower-end)
        ...
        if (canvas) {
            ...
            // Use lower resolution and simpler rendering for mobile/low-end devices
            const renderer = new THREE.WebGLRenderer({ 
                canvas: canvas, 
                antialias: !lowEndDevice, // Disable antialiasing on low-end devices
                powerPreference: lowEndDevice ? "low-power" : "high-performance",
                alpha: true
            });
            
            // Set pixel ratio based on device capability
            const pixelRatio = lowEndDevice ? 1 : Math.min(window.devicePixelRatio, isMobile ? 1.5 : 2);
            renderer.setPixelRatio(pixelRatio);
            renderer.setSize(window.innerWidth, window.innerHeight);

            // --- POST-PROCESSING (BLOOM) - Disable for low-end devices ---
            let composer = null;
            let bloomPass = null;
            ...
            if (!lowEndDevice) {
                ...
                bloomPass = new UnrealBloomPass(
                    ...
                    isMobile ? 1.0 : 1.8, // Lower bloom intensity on mobile
                    0.6, 
                    0.4
                );
            ...
            let particleCount = isMobile ? (lowEndDevice ? 5000 : 10000) : 30000;
            
            let parameters = {
                count: particleCount,
                size: lowEndDevice ? 0.03 : 0.02, // Slightly larger particles on low-end devices
                radius: 8,
                branches: 5,
                spin: 1.5,
                randomness: 0.5,
            ...
            let frameCount = 0;
            // Lower FPS on mobile and low-end devices for better performance
            const maxFPS = lowEndDevice ? 30 : (isMobile ? 45 : 60);
            let then = 0;
            const interval = 1000 / maxFPS;
            
            // Only update a portion of particles each frame for mouse interaction
            const particlesPerFrame = Math.floor(parameters.count / (lowEndDevice ? 20 : 10)); // Update 5-10% of particles per frame
            let particleIndex = 0;
            ...
            const animate = (now) => {
                ...
                // Use the composer if available, otherwise use the regular renderer
                if (composer && !lowEndDevice) {
                    composer.render();
                } else {
            ...
            const initSolarSystem = () => {
                ...
                solarSystemRenderer.setPixelRatio(lowEndDevice ? 1 : Math.min(window.devicePixelRatio, isMobile ? 1.5 : 2));
                ...
                // --- POST-PROCESSING (BLOOM EFFECT) - Skip for low-end devices ---
                let solarComposer = null;
                let solarBloomPass = null;
                
                if (!lowEndDevice) {
                    const renderScene = new RenderPass(solarSystemScene, solarSystemCamera);
                    solarBloomPass = new UnrealBloomPass(new THREE.Vector2(window.innerWidth, window.innerHeight), 1.5, 0.4, 0.85);
                    solarBloomPass.threshold = 0;
                    solarBloomPass.strength = isMobile ? 1.0 : 1.2; // Reduce bloom strength on mobile
                    solarBloomPass.radius = 0.5;

                    solarComposer = new EffectComposer(solarSystemRenderer);
                    solarComposer.addPass(renderScene);
                    solarComposer.addPass(solarBloomPass);
                }
                ...
                // Planet data: name, size, color, distance, speed, hasRing
                const planetData = [
                    ['Mercury', 1.5, 0x9a7f6f, 25, 0.5, false],
                    ['Venus', 2.2, 0xd4a05f, 35, 0.4, false],
                    ['Earth', 2.5, 0x5f8ad4, 45, 0.3, false],
                    ['Mars', 2.0, 0xd45f5f, 55, 0.25, false],
                    ['Jupiter', 4.5, 0xd4a05f, 70, 0.2, false],
                    ['Saturn', 4.0, 0xd4af37, 85, 0.15, true],
                    ['Uranus', 3.0, 0x5f8ad4, 100, 0.1, false],
                    ['Neptune', 3.2, 0x1b3984, 115, 0.08, false]
                ];

                // Function to create a planet system
                function createPlanet(name, size, color, distance, speed, hasRing = false) {
                    const pivot = new THREE.Object3D();
                    solarSystemScene.add(pivot);

                    // Use fewer segments for spheres on low-end devices
                    const segments = lowEndDevice ? 16 : 32;
                    const planetMaterial = new THREE.MeshStandardMaterial({ color: color, metalness: 0.3, roughness: 0.6 });
                    const planet = new THREE.Mesh(new THREE.SphereGeometry(size, segments, segments), planetMaterial);
                    ...
                    // Orbital Path (visual only) - skip on low-end devices
                    if (!lowEndDevice) {
                        const pathGeometry = new THREE.BufferGeometry().setFromPoints(
                            new THREE.Path().absarc(0, 0, distance, 0, Math.PI * 2, false).getSpacedPoints(50)
                        );
                ...
                function createSolarSystem() {
                    // --- CENTRAL SUN ---
                    const segments = lowEndDevice ? 32 : 64;
                    const sunMaterial = new THREE.MeshBasicMaterial({ color: 0xffdd88 });
                    const sun = new THREE.Mesh(new THREE.SphereGeometry(12, segments, segments), sunMaterial);
                ...
                solarSystemRenderer.domElement.addEventListener('click', (event) => {
                    const mouse = new THREE.Vector2();
                    ...
                    if (intersects.length > 0) {
                        ...
                        // Traverse up to find the planet mesh
                        ...
                        if (planet.userData && planet.userData.name) {
                            ...
                            const planetDescriptions = {
                                ...
                                'Earth': 'Our home planet, the only astronomical object known to harbor life.',
                                ...
                                'Saturn': 'The sixth planet from the Sun, known for its spectacular ring system.',
                            ...
                            descElement.textContent = planetDescriptions[planetName] || 'A planet in our solar system.';
                ...
                document.querySelector('.close-info').addEventListener('click', () => {
                    document.querySelector('.planet-info').classList.remove('visible');
                });
                ...
                const animateSolarSystem = () => {
                    ...
                    // Use the composer if available, otherwise use the regular renderer
                    if (solarComposer && !lowEndDevice) {
                        solarComposer.render();
                    } else {
                ...
                window.addEventListener('resize', () => {
                    ...
                    solarSystemRenderer.setSize(window.innerWidth, window.innerHeight);
                    if (solarComposer) solarComposer.setSize(window.innerWidth, window.innerHeight);
                });
                ...
                if (isMobile) {
                    ...
                    document.getElementById('mobile-pause').addEventListener('click', () => {
                        ...
                        objects.forEach(objData => {
                            if (rotationPaused) {
                                objData.originalSpeed = objData.speed;
                                objData.speed = 0;
                            } else {
                                objData.speed = objData.originalSpeed || objData.speed;
                            }
                        });
                    });
                    ...
                    document.getElementById('mobile-speed').addEventListener('click', () => {
                        ...
                        objects.forEach(objData => {
                            if (objData.type === 'orbit') {
                                objData.speed = objData.originalSpeed * speedMultiplier;
                            }
                        });
                    });
                }
